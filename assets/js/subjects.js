...
function setDataToForm(sid) {
    ...
    fetch('../assets/fetchSubjectInfo.php', {
        ...
        body: 'department_id=' + encodeURIComponent(sid),
    })
        ...
        .then(data => {
            ...
            document.getElementById("Course-edited-name").value = data['subject'];
...
(() => {
    ...
    const addSubjectForm = document.getElementById("create-course-form");
    const addSubjectBtn = document.getElementById("add_course_btn");
    const editSubjectForm = document.getElementById("editCourseForm");
    ...
    document.getElementById("Course-edited-name").addEventListener("keydown", function (event) {
        if (event.key === 'Enter') {
            saveBtnClicked();
        }
    }, false);

    document.getElementById('save-new-course-name').addEventListener('click', event => {
        saveBtnClicked();
